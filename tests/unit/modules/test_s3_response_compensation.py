...
def test_s3_generates_beta_list_with_fb_copy_and_ratio_rules() -> None:
...
def test_s3_uses_distance_loss_model_for_eb_compensation() -> None:
    ...
    assert out.Beta_list["EB"] == pytest.approx(1.4814814814814812)
...
def test_s3_uses_impulse_rate_for_fsb_compensation() -> None:
    fast_payload = make_valid_bogie_payload()
    fast_payload["response_time"]["FSB"]["impulse_rate"] = 2.5
    fast_ctx = Context(validated_inputs=Inputs.model_validate(fast_payload))
    fast_ctx = run_s2(fast_ctx)

    slow_payload = make_valid_bogie_payload()
