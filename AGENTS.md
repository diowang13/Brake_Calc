...
本仓库实现城轨列车**制动压力标准计算**的确定性工作流：给定列车参数与制动需求，输出按 `load_group × brake_type × controller` 组织的压力标准矩阵。

- **业务真相源**：[城轨制动计算 Workflow Spec（v1.0 草案）](https://www.notion.so/Workflow-Spec-v1-0-495ad4a24779422ca99d9830f40b68e1?pvs=21)（也镜像在 `specs/brake-workflow.md`）。代码实现必须与 spec 一致，发现歧义先澄清 spec 再改代码。
- **运行形态**：
...
src/brake_calc/
  ...
  domain/      # 纯计算函数（运动学、质量模型、分配、机械模型、k(f) 校准）
  workflow/    # runner + workflow.yaml（执行顺序，对应 spec §8）
  io/          # 配置加载、报表输出
  cli.py       # 本地命令行入口
configs/       # 示例输入、默认 k_config、项目配置
...
- **集成测试**：`tests/integration/test_workflow_end_to_end.py` 用 `tests/fixtures/mathcad_reference.yaml` 做黄金对比，数值误差阈值 `rtol=1e-4`
...
1. **读 spec**：`specs/brake-workflow.md` 对应章节（不是 Notion 链接；本地文件是权威副本）
...
> 本文件是仓库层面的 agent 规则；业务逻辑请查阅 `specs/brake-workflow.md`。
> 
